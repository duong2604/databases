...
-- 2005.
select e.first_name, e.hire_date  from employees e  where year(e.hire_date) >= '2002' and year(e.hire_date) < '2005';

-- 3. Display first name and join date of the employees who is either IT Programmer or
-- Sales Man.
select e.first_name, e.hire_date from employees e, jobs j  where e.job_id = j.job_id and j.job_title = 'Programmer' or j.job_title = 'Sales Manager'

select e.first_name, e.hire_date from employees e join jobs j on e.job_id  = j.job_id where
exists (select j2.job_title from jobs j2 where  j2.job_title = 'Programmer' or j2.job_title = 'Sales Manager' )
